...
/*   Updated: 2018/12/21 15:27:20 by ayguillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		print_long(va_list ap, int nb, t_flag flag)
{
	...
	ft_nblen(var1, 10, &nb);
	size = nb;
	print_flag_plus_space1_uns(flag.sign, flag.zero, (ULL*)&var1, (ULL*)&nb);
	if (flag.sign != 3 || flag.precis )
		flag_space(flag.space, nb, flag.zero, &nb);
	print_flag_plus_space_uns(flag.sign, flag.zero, (ULL*)&var1);
